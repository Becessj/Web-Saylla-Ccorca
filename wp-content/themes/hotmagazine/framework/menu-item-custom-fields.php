 ...
 * @since 0.1.0
 */
class hotmagazine_Menu_Item_Custom_Fields {
	...
	public static function init() {
		...
		self::$fields = array(
			...
			'mega' => esc_html__( 'Mega DropDown', 'hotmagazine' ),
		);
	}
		...
		foreach ( self::$fields as $_key => $label ) :
			...
			?>
				<p class="description description-wide <?php echo esc_attr( $class ) ?>">
					<label for="<?php echo esc_attr($id); ?>"><?php echo esc_html($label); ?><br />
					<select id="<?php echo esc_attr($id); ?>" class="widefat <?php echo esc_attr($id); ?>" name="<?php echo esc_attr($name); ?>">
						...
						<option value="default" <?php if($value=='default'){ echo "selected"; } ?>>-Select Widget-</option>
					  <?php foreach($pages as $p){ ?>
					   <option value="<?php echo  esc_attr($p->ID) ; ?>" <?php if($value==$p->ID){ echo "selected"; } ?>>
					   	 <?php echo ucwords( $p->post_title ); ?>
						     </option>
						<?php } ?>
					  
					</select>
					<br/>
					<span class="td-wpa-info"><strong>Just a tip:</strong> If you choose to load a mega menu, please do not add submenus to this item. The mega menu and mega page menu have to be the top most menu item. </span>
...
hotmagazine_Menu_Item_Custom_Fields::init();
